...
class PageNavigation:
    ...
    def add_page_navigation(self, places: list, is_index: bool, *category_id: int):
    ...
    def pages_info(self, total: int) -> list:
        ...
        filtered_list_of_pages: list = sorted(
            [i for i in [page2left, page1left, self.page_id, page1right, page2right] if i])
        for i, j in enumerate(filtered_list_of_pages):
            if (pervpage[-1] or nextpage[-1]) and (
                    type(j) == int and i + 1 < len(filtered_list_of_pages) and j + 1 != filtered_list_of_pages[i + 1]):
        ...
        return [(i, True) if type(i) == int else i for i in
                [i if i != self.page_id else (self.page_id, None) for i in filtered_list_of_pages]]
